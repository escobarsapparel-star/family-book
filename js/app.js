...
function go(r,opts={}){if(r==="profile"){const id=currentMemberId();if(id)r="view-member:"+id}if((currentRoute==="family-fun"||currentRoute==="family-fun-gallery")&&r!==currentRoute)window.FB_FAMILY_FUN?.cleanup?.();currentRoute=r;let navRoute=(r==="albums"||r==="new-album"||r.startsWith("album:")||r.startsWith("edit-album:"))?"memories":(r==="add-history"||r.startsWith("edit-history:"))?"tree":r==="family-fun-gallery"?"family-fun":r;document.querySelectorAll(".nav").forEach(b=>b.classList.toggle("active",b.dataset.r===navRoute));$("#screen").innerHTML=r==="home"?home():r==="memories"?FB_MEMORIES.pageShell():r==="albums"?FB_ALBUMS.pageShell():r==="new-album"?FB_ALBUMS.editorShell():r.startsWith("edit-album:")?FB_ALBUMS.editorShell(r.split(":")[1]):r.startsWith("album:")?FB_ALBUMS.detailShell(r.split(":")[1]):r==="family-fun"?FB_FAMILY_FUN.pageShell():r==="family-fun-gallery"?FB_FAMILY_FUN.galleryShell():r==="calendar"?FB_CALENDAR.pageShell():r==="add-event"?FB_CALENDAR.editorShell():r.startsWith("edit-event:")?FB_CALENDAR.editorShell(r.split(":")[1]):r.startsWith("view-event:")?FB_CALENDAR.detailShell(r.slice("view-event:".length)):r==="add-memory"?FB_MEMORIES.editorShell():r.startsWith("view-memory:")?FB_MEMORIES.detailShell(r.split(":")[1]):r.startsWith("edit-memory:")?FB_MEMORIES.editorShell(r.split(":")[1]):r==="members"?members():r==="add-member"?addMember():r.startsWith("edit-member:")?editMember(r.split(":")[1]):r==="add-history"?historyForm():r.startsWith("edit-history:")?historyForm(r.split(":")[1]):r.startsWith("family-unit:")?familyUnitView(r.split(":")[1]):r.startsWith("view-member:")?viewMember(r.split(":")[1]):r.startsWith("member-wall:")?memberWallPage(r.split(":")[1]):r==="tree-full"?fullFamilyTree():r==="tree"?familyTree():r==="story-cover"?storyCoverPage():r==="story-slideshow"?storySlideshowPage():r==="notifications"?FB_NOTIFICATIONS.pageShell():r==="family-access"?FB_INVITES.pageShell():r==="settings"?FB_SETTINGS.pageShell():r==="profile"?profilePage():page(r);document.querySelectorAll("#screen [data-r]").forEach(b=>b.onclick=()=>go(b.dataset.r));
 ...
 if(r.startsWith("edit-member:")){
  ...
  if(f)f.onsubmit=async e=>{e.preventDefault();clearFamilySafetyMessage(f);let list=ensureOwner(),idx=list.findIndex(x=>x.id===id);if(idx<0)return;
   ...
   const name=`${$("#mfFirst").value.trim()} ${$("#mfLast").value.trim()}`.trim(),birthday=$("#mfBirthday").value,sex=$("#mfSex")?.value||"",plan=relationshipPlanRows(f);
   ...
   if(allowed==="structure"){
     list[idx]={...list[idx],name,birthday,sex};
   }else{
 ...
 if(f)f.onsubmit=async e=>{e.preventDefault();clearFamilySafetyMessage(f);let first=$("#mfFirst").value.trim(),last=$("#mfLast").value.trim(),photo=$("#mfPhotoData")?.value||"";
  let list=ensureOwner(),id=crypto.randomUUID(),name=`${first} ${last}`.trim(),birthday=$("#mfBirthday").value,sex=$("#mfSex")?.value||"",plan=relationshipPlanRows(f);
  const guard=validateRelationshipPlan(id,plan,true);
  if(!guard.ok){showFamilySafetyMessage(f,guard.message);return}
 ...
 window.FB_EVENT_DAY?.bindRoute?.(r);
...
function memberSexOptions(value=""){
 ...
 return `<option value="" ${!current?"selected":""}>Not specified</option><option value="male" ${current==="male"?"selected":""}>Male</option><option value="female" ${current==="female"?"selected":""}>Female</option><option value="other" ${current==="other"?"selected":""}>Other</option>`;
}
...
function editMember(id){
 ...
 <form id="editMemberForm" data-member-id="${esc(id)}" data-edit-mode="${mode}"><div class="field-row"><label>First name<input id="mfFirst" required value="${esc(first)}"></label><label>Surname<input id="mfLast" required value="${esc(last)}"></label></div><div class="field-row"><label>Birthday<input id="mfBirthday" type="date" value="${esc(m.birthday||"")}"></label><label>Sex <span class="optional">(optional)</span><select id="mfSex">${memberSexOptions(m.sex||"")}</select></label></div>${structureOnly?`<div class="field-row"><div class="locked-member-field"><span>Email</span><strong>${esc(m.email||"Not added")}</strong><small><i data-lucide="lock"></i>Managed by this member</small></div><div class="locked-member-field"><span>Contact number</span><strong>${esc(m.phone||"Not added")}</strong><small><i data-lucide="lock"></i>Managed by this member</small></div></div>`:`<div class="field-row"><label>Email <span class="optional">(optional)</span><input id="mfEmail" type="email" value="${esc(m.email||"")}"></label><label>Contact number <span class="optional">(optional)</span><input id="mfPhone" type="tel" placeholder="e.g. 082 123 4567" autocomplete="tel" value="${esc(m.phone||"")}"></label></div>`}
